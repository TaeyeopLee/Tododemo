...
public class TodoController {
    ...
    public ResponseEntity<?> createTodo(@RequestBody TodoDTO dto) {
        try {
            ...
            // (3) 임시 사용자 id를 설정해 준다. 이 부분은 4장에서 수정예정. 지금은 인증과 인가기능이 없으므로 한 사용자(temporary-user)만 로그인 없이 사용할 수 있는 애플리케이션인 셈.
            entity.setUserId(temporaryUserId);

            // (4) 서비스를 이용해 Todo 엔티티를 생성한다.
            List<TodoEntity> entities = service.create(entity);

            // (5) 자바 스트림을 이용해 리턴된 엔티티를 TodoDTO 리스트로 변환한다.
    ...
    public ResponseEntity<?> retrieveTodoList() {
        ...
        // (1) 서비스 메소드의 retrieve() 메소드를 사용해 Todo 리스트를 가져온다.
    ...
    public ResponseEntity<?> deleteTodo(@RequestBody TodoDTO dto) {
        try {
            ...
            // (2) 임시 사용자 아이디를 설정해 준다. 이 부분은 4장에서 수정예정.
            entity.setUserId(temporaryUserId);
